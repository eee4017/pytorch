...
void InputBuffer::add(
  ...
  if (!opt_accum_streams[pos].has_value()) {
    ...
    const bool syncs_with_capturing_stream =
        opt_producer_stream->is_capturing() || ready_stream->is_capturing();
    const bool accum_on_third_device =
        device != opt_consumer_stream->device() &&
        device != opt_producer_stream->device();
    const bool sync_crosses_capture_boundary = !accum_stream->is_capturing() &&
        syncs_with_capturing_stream && accum_on_third_device;
    TORCH_CHECK(
        !sync_crosses_capture_boundary,
        "During CUDA graph capture, autograd node '",
        fn->name(),
