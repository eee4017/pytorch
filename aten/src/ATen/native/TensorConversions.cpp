...
static inline Tensor FN_to_impl(const Tensor& self, const TensorOptions& options, bool non_blocking, bool copy, bool csr) {
  auto memory_format = options.memory_format_opt().value_or(MemoryFormat::Preserve);
  ...
  // See Note [Explicit nullopt MemoryFormat argument]
  auto r = at::empty(self.sizes(), options.memory_format(memory_format), c10::nullopt);
  auto tID = self.getIntrusivePtr().get()->tID;
  r.copy_(self, non_blocking);
  r.unsafeGetTensorImpl()->tID = tID;
  return r;
}
...
Tensor FN_to(const Tensor& self, const TensorOptions& options_, bool non_blocking, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format, bool csr) {
  ...
  TORCH_CHECK(options.requires_grad_opt() == c10::nullopt,
    "FN_to(options) expects unset requires_grad flag, but got "
    "options.requires_grad set as ", options.requires_grad());

  const auto & layout_opt = options.layout_opt();
  TORCH_CHECK(!layout_opt || self.layout() == layout_opt.value(),
    "FN_to(options) doesn't support converting to a different layout, "
    "but got self.layout being ", self.layout(),
    " and options.layout set as ", options.layout());
  ...
  return FN_to_impl(self, specified_options.memory_format(optional_memory_format), non_blocking, copy, csr);
}

Tensor FN_to(const Tensor& self, Device device, ScalarType dtype, bool non_blocking, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format, bool csr) {
  device = ensure_has_index(device);
  return FN_to_impl(
      self,
      self.options().device(device).dtype(dtype).memory_format(optional_memory_format),
...
Tensor FN_to(const Tensor& self, ScalarType dtype, bool non_blocking, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format, bool csr) {
  return FN_to_impl(
      self, self.options().dtype(dtype).memory_format(optional_memory_format), non_blocking, copy, csr);
}

Tensor FN_to(const Tensor& self, const Tensor& other, bool non_blocking, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format, bool csr) {
  auto options = other.options();
  return FN_to_impl(self, options.memory_format(optional_memory_format), non_blocking, copy, csr);
}
