...
class CudaLmsStorageImpl : public at::LmsStorageImpl {
 ...
 protected:
  ...
  void do_pageout(void* dst, const void* src, size_t size, bool speculative) override {
    std::string message("pageout size = ");
    message += std::to_string(size);
    LMSSettings::debug_log(1, this, message.c_str());
  ...
  void do_pagein(void* dst, const void* src, size_t size, bool pin) override {
    std::string message("pagein size = ");
    message += std::to_string(size);
    LMSSettings::debug_log(1, this, message.c_str());
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  Block* malloc(int device, size_t size, cudaStream_t stream, LMSSettings* lms)
  {
    ...
    //std::cerr << "reportMemoryUsageToProfiler " << size << " " << curr_time << "\n";
  ...
  void free(Block* block)
  {
    ...
    //time_t curr_time = std::chrono::system_clock::now().time_since_epoch().count();
    //std::cerr << "reportMemoryUsageToProfiler " << ((int64_t)(-block->size)) << " " << curr_time << "\n";
